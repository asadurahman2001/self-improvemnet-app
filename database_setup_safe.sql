-- Safe database setup for Self Improvement App
-- Run this in your Supabase SQL Editor - it won't fail if tables/policies already exist
...
-- Create prayer_records table (if it doesn't exist)
...
-- Create quran_sessions table (if it doesn't exist)
...
-- Create habit_records table (if it doesn't exist)
...
-- Create sleep_records table (if it doesn't exist)
...
-- Create RLS policies for study_sessions (only if they don't exist)
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'study_sessions' AND policyname = 'Users can delete their own study sessions') THEN
    CREATE POLICY "Users can delete their own study sessions" ON study_sessions FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- Create RLS policies for prayer_records (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'prayer_records' AND policyname = 'Users can view their own prayer records') THEN
    CREATE POLICY "Users can view their own prayer records" ON prayer_records FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'prayer_records' AND policyname = 'Users can insert their own prayer records') THEN
    CREATE POLICY "Users can insert their own prayer records" ON prayer_records FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'prayer_records' AND policyname = 'Users can update their own prayer records') THEN
    CREATE POLICY "Users can update their own prayer records" ON prayer_records FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'prayer_records' AND policyname = 'Users can delete their own prayer records') THEN
    CREATE POLICY "Users can delete their own prayer records" ON prayer_records FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- Create RLS policies for quran_sessions (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'quran_sessions' AND policyname = 'Users can view their own quran sessions') THEN
    CREATE POLICY "Users can view their own quran sessions" ON quran_sessions FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'quran_sessions' AND policyname = 'Users can insert their own quran sessions') THEN
    CREATE POLICY "Users can insert their own quran sessions" ON quran_sessions FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'quran_sessions' AND policyname = 'Users can update their own quran sessions') THEN
    CREATE POLICY "Users can update their own quran sessions" ON quran_sessions FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'quran_sessions' AND policyname = 'Users can delete their own quran sessions') THEN
    CREATE POLICY "Users can delete their own quran sessions" ON quran_sessions FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- Create RLS policies for habit_records (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'habit_records' AND policyname = 'Users can view their own habit records') THEN
    CREATE POLICY "Users can view their own habit records" ON habit_records FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'habit_records' AND policyname = 'Users can insert their own habit records') THEN
    CREATE POLICY "Users can insert their own habit records" ON habit_records FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'habit_records' AND policyname = 'Users can update their own habit records') THEN
    CREATE POLICY "Users can update their own habit records" ON habit_records FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'habit_records' AND policyname = 'Users can delete their own habit records') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'attendance_records' AND policyname = 'Users can view their own attendance records') THEN
    CREATE POLICY "Users can view their own attendance records" ON attendance_records FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'attendance_records' AND policyname = 'Users can insert their own attendance records') THEN
    CREATE POLICY "Users can insert their own attendance records" ON attendance_records FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'attendance_records' AND policyname = 'Users can update their own attendance records') THEN
    CREATE POLICY "Users can update their own attendance records" ON attendance_records FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'attendance_records' AND policyname = 'Users can delete their own attendance records') THEN
    CREATE POLICY "Users can delete their own attendance records" ON attendance_records FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- Create RLS policies for class_schedules (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'class_schedules' AND policyname = 'Users can view their own class schedules') THEN
    CREATE POLICY "Users can view their own class schedules" ON class_schedules FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'class_schedules' AND policyname = 'Users can insert their own class schedules') THEN
    CREATE POLICY "Users can insert their own class schedules" ON class_schedules FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'class_schedules' AND policyname = 'Users can update their own class schedules') THEN
    CREATE POLICY "Users can update their own class schedules" ON class_schedules FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'class_schedules' AND policyname = 'Users can delete their own class schedules') THEN
    CREATE POLICY "Users can delete their own class schedules" ON class_schedules FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- Create RLS policies for sleep_records (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'sleep_records' AND policyname = 'Users can view their own sleep records') THEN
    CREATE POLICY "Users can view their own sleep records" ON sleep_records FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'sleep_records' AND policyname = 'Users can insert their own sleep records') THEN
    CREATE POLICY "Users can insert their own sleep records" ON sleep_records FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'sleep_records' AND policyname = 'Users can update their own sleep records') THEN
    CREATE POLICY "Users can update their own sleep records" ON sleep_records FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'sleep_records' AND policyname = 'Users can delete their own sleep records') THEN
    CREATE POLICY "Users can delete their own sleep records" ON sleep_records FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- Create RLS policies for exams (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exams' AND policyname = 'Users can view their own exams') THEN
    CREATE POLICY "Users can view their own exams" ON exams FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exams' AND policyname = 'Users can insert their own exams') THEN
    CREATE POLICY "Users can insert their own exams" ON exams FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exams' AND policyname = 'Users can update their own exams') THEN
    CREATE POLICY "Users can update their own exams" ON exams FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'exams' AND policyname = 'Users can delete their own exams') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_sleep_records_updated_at') THEN
